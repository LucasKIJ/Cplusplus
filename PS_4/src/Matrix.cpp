...
Matrix::Matrix(int row, int col) : mRow(row), mCol(col), mSize(row * col) {
  ...
  if (mCol < 1 || mRow < 1) {
    throw Exception("Matrix::Matrix - Invalid dimensions",
                    "Please ender dimension greater or equal"
                    "to one.");
  }
 ...
 */
  ...
  if (row < 0 || row >= mRow || col < 0 || col >= mCol) {
    throw Exception(
        "Matrix::setValue - Out of range",
        "The element coordinates given when"
...
Matrix operator+(const Matrix &m1, const Matrix &m2) {
  ...
  // If input matrices not of same dimensions throw error
    ...
    throw Exception("Different dimensions",
                    "Matrix add - Matrices are different dimensions\n");
  }
}
 ...
 */
  ...
  if (m1.mRow == m2.mRow && m1.mCol == m2.mCol) {
    ...
    throw Exception("Different dimensions",
                    "Matrix subtract - Matrices are different dimensions\n");
  }
}
...
Matrix operator*(const Matrix &m1, const Matrix &m2) {
  ...
  if (m1.mCol == m2.mRow) {
    ...
    throw Exception("Different dimensions",
                    "Matrix product - Number of columns in the first matrix"
...
double dot(const Matrix &m1, const Matrix &m2) {
  ...
  if ((m1.mCol == m1.mSize || m1.mRow == m1.mSize) &&
      (m2.mCol == m2.mSize || m2.mRow == m2.mSize) && (m1.mSize == m2.mSize)) {
    double prod = 1;
...
Matrix &Matrix::operator=(const Matrix &m) {
  ...
  if (mRow != m.mRow && mCol != m.mCol) {
    throw Exception("Dimension mismatch",
                    "Matrix assignment operator - Matrices"
...
double norm(Matrix &m, int p) { return m.norm(p); }

double norm(Matrix &m, std::string p) { return m.norm(p); }
// Member method
// calculate p-norm of a Matrix v
double Matrix::norm(int p) const {
  ...
  if (p == 2) {
    ...
    throw Exception("Matrix::norm - norm selected not available",
                    "Choice of norm not available,"
                    " please choose from the 1-norm (p=1), 2-norm (p=2), "
                    "infinity-norm (p=\"inf\")"
...
int Matrix::argRowMax(int row, int col, bool abs_true) const {
  /** @brief Find argument of maximum value in row, by default this is the
   * absolute max
...
int Matrix::argColMax(int col, int row, bool abs_true) const {
  /** @brief Find argument of maximum value in column, by default this is the
   * absolute max
...
std::tuple<Matrix, Matrix, Matrix, int> lu(const Matrix &A) {
   ...
   *       Matrix U - upper triangular matrix, int sign - number of permutation
   * made
   */
  ...
  if (m != n) {
    throw Exception("LU - Non-square matrix input",
                    "LU decomposition is"
...
double Matrix::det() const {
  ...
  // Check if dimension are equal
  if (mRow != mCol) {
    throw Exception("Matrix::det() - Non-square matrix",
                    "Matrix entered is non-square."
...
std::tuple<Matrix, Matrix> qr(const Matrix &A, bool reduced) {
  ...
  if (m < n) {
    throw Exception(
        "qr - Columns greater than rows",
        "This implementation of QR factorisation"
...
Matrix hessenbergReduction(const Matrix &A) {
  ...
  if (m > 2) {
    ...
    for (int k = 0; k < m - 2; k++) {
      ...
      g = (double)((x.getValue(k + 1) >= 0) - (x.getValue(k + 1) < 0)) *
          norm(x);
...
Matrix eigenVal(const Matrix &A, double tol) {
  ...
  if (A.mRow != A.mCol) {
    throw Exception("eigenSystem - Non-square matrix",
  ...
  for (int k = m; k > 0; k--) {
    ...
    while (fabs(H.getValue(k - 1, k - 2)) > tol) {
      ...
      mu = H.getValue(k - 2, k - 1) -
           (double)((delta > 0) - (delta < 0)) *
               pow(H.getValue(k - 1, k - 1), 2) /
               (fabs(delta) +
                pow(pow(delta, 2) + pow(H.getValue(k - 1, k - 1), 2), 0.5));
...
Matrix gaussianElimination(const Matrix &A, const Matrix &b) {
  ...
  // Check if A is square
  if (A.mCol > A.mRow) {
    throw Exception("gaussianElimination - Underdetermined",
